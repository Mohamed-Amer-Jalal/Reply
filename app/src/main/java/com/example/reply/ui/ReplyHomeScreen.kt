...
fun ReplyHomeScreen(
    ...
    when (navigationType == ReplyNavigationType.PERMANENT_NAVIGATION_DRAWER) {
        true -> PermanentNavigationDrawer(
            drawerContent = {
        ...
        false -> when (replyUiState.isShowingHomepage) {
            true -> ReplyAppContent(
                navigationType = navigationType,
                contentType = contentType,
                replyUiState = replyUiState,
                onTabPressed = onTabPressed,
...
private fun ReplyAppContent(
    ...
    Row(modifier = modifier) {
        ...
        Column(
            ...
            when (contentType == ReplyContentType.LIST_AND_DETAIL) {
                true -> ReplyListAndDetailContent(
                    replyUiState = replyUiState,
                    onEmailCardPressed = onEmailCardPressed,
                    modifier = Modifier.weight(1f)
                )

                false -> ReplyListOnlyContent(
                    replyUiState = replyUiState,
                    onEmailCardPressed = onEmailCardPressed,
                    modifier = Modifier
                        .weight(1f)
